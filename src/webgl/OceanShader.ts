...
export const oceanFragmentShader = `
  ...
  // COLOR TWEAKS FOR TIFFANY BLUE & PINK
  const vec3 waterCol = vec3(0.50, 0.85, 0.82); // Tiffany Blue
  const float waterAbsorp = 0.7;
  const vec3 subsurfCol = waterCol * vec3(1.1, 1.2, 1.0);
  const vec3 groundCol = vec3(0.85, 0.65, 0.70); // Pink sand
  ...
  float WaterHeight(vec3 p, int waveCount) {
      return WaveHeight(p.xz * 0.1, uTime, waveCount) + oceanHeight;
  }
  ...
  void DarkenGround(inout vec3 col, vec3 groundPos, float oceanHeight, out float wetness) {
      ...
      // slightly darker, more saturated pink when wet
      col = mix(col, col * vec3(0.95, 0.85, 0.90) * 0.8, wetness);
  }
  ...
  vec3 Reflection(vec3 refl, float fresnel) {
      ...
      // Simple procedural sky reflection 
      vec3 skyCol = mix(vec3(0.4, 0.7, 0.9), vec3(1.0), max(0.0, (1.0 - refl.y) * 0.5));
      col += fresnel * skyCol * 0.4;
      return col;
  }
  ...
  vec3 Render(float t, vec3 ro, vec3 rd) {
      ...
      if (material == MAT_OCEAN) {
          ...
          vec3 nor = Normal(p);
          nor = normalize(mix(nor, vec3(0,1,0), nearShoreAlpha * 0.9));
          refl = reflect(rd, nor);
